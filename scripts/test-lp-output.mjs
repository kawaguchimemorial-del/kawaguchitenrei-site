...
//
// このスクリプトが測るのは HTML 本体だけで、外部スクリプト（GTM 等）は含まない。
// 2026-09-11 の松澤判断により、計測・広告タグの外部スクリプトは HTML の予算とは
// 別枠で許可される（非同期・/lp/ 配下限定・計測広告目的に限る）。
// HTML 本体の 50KB は緩めないため、この判定はこれまでどおり維持する。
//
