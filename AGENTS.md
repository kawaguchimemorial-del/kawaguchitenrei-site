...
- 変更後は `node scripts/test-lp-output.mjs` を実行する。ページ重量は**HTML本体の brotli 圧縮後 50KB 以下**で判定する（CLAUDE.md §21.2・2026-09-06 松澤判断）。非圧縮のバイト数は記録するだけで基準にしない。**計測・広告タグの外部スクリプトは 2026-09-11 の松澤判断により別枠で許可**（非同期・`/lp/` 配下限定・計測広告目的に限る）。HTML本体の 50KB は緩めない。
- LPと本サイトは同じGA4の入口セッションで分離する。`scripts/ad-lp-report.mjs` は読み取り専用。既存SEO監査のGA4全体値が自動で分離されたわけではない。**`lp_*` は既存Googleタグへ gtag コマンド形式で直接送出するので、同名のGTMタグを重ねない。** 2026-09-11 に data layer イベント形式へ変更しGTMに転送タグを足す方針を立てたが、**2026-09-12 の実測で誤りと判明し撤去した**（転送タグを足すと全イベントが2件ずつ記録される）。本番は一貫して gtag コマンド形式で動いており、それで GA4 に届いている。訂正の経緯は `docs/ad-lp/2026-09-11-lp-event-fix.md` 冒頭。`generate_lead` と `lp_generate_lead`、`click_tel` と `lp_click_tel` を合算しない点は変わらない。
