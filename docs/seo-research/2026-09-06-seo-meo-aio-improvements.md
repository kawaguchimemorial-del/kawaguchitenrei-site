...
- 完了: Googleビジネスプロフィールの一日葬商品を通常価格496,000円へ訂正し、会員価格396,000円・税込・別途費用の説明と公式プランURLを追加。管理画面は公開申請後「反映待ち」。
- 完了: 社内利用・開発環境・テスト用ブラウザの計測除外を実装。
- 公開確認中: mainへのpush、Vercel反映、Search Consoleへの送信結果は下記の公開記録へ追記する。
...
公開処理中。確認済みのコミット・デプロイ・Search Console結果は完了後に追記する。

## 残る課題と観測

- Google商品の「反映待ち」と、GSCリクエスト後の実登録はGoogle側の処理。即時反映・順位・AI引用を保証しない。
