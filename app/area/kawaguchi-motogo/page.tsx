...
import { areaKawaguchiMotogo } from "@/lib/areas";
import { AreaLocalGuide } from "@/components/area/AreaLocalGuide";
import { buildLocalGuide } from "@/lib/area-local-guide";

const SITE_URL = "https://kawaguchitenrei.com";
const pageUrl = `${SITE_URL}/area/${areaKawaguchiMotogo.slug}/`;
const imageUrl = `${SITE_URL}/images/home/hall/hall-exterior.jpg`;

export const metadata: Metadata = {
  title: areaKawaguchiMotogo.metaTitle,
  description: areaKawaguchiMotogo.metaDescription,
  alternates: { canonical: `/area/${areaKawaguchiMotogo.slug}/` },
  openGraph: {
    title: areaKawaguchiMotogo.metaTitle,
    description: areaKawaguchiMotogo.metaDescription,
    url: `/area/${areaKawaguchiMotogo.slug}/`,
    type: "article",
    images: [
      {
...
const faqJsonLd = {
  "@context": "https://schema.org",
  "@type": "FAQPage",
  mainEntity: areaKawaguchiMotogo.faqs.map((f) => ({
    "@type": "Question",
    name: f.q,
    acceptedAnswer: {
      "@type": "Answer",
...
const funeralHomeJsonLd = {
  ...
  description:
    "埼玉高速鉄道「川口元郷駅」周辺で葬儀・家族葬・直葬・一日葬を承る葬儀社。自社式場「川口メモリアルホール」(川口市西新井宿)を中心に、ご搬送・ご安置・葬儀式・川口市めぐりの森での火葬まで一貫してサポートします。",
};
...
export default function KawaguchiMotogoAreaPage() {
  return (
    <>
      <script
      ...
      <AreaDetailIntro area={areaKawaguchiMotogo} />
      <AreaPrimaryHall
        area={areaKawaguchiMotogo}
        description="川口元郷駅周辺の地域から、お車でご来館いただける川口典礼の自社ホール。1日1組貸切の落ち着いた式場で、家族葬・一日葬に適しています。"
      />
      <AreaFeatures area={areaKawaguchiMotogo} />
      <AreaPlans area={areaKawaguchiMotogo} />
      <AreaSaijo area={areaKawaguchiMotogo} />
      <AreaReasons area={areaKawaguchiMotogo} />
      <AreaFlow area={areaKawaguchiMotogo} />
      <AreaFaq area={areaKawaguchiMotogo} />

      <AreaLocalGuide {...buildLocalGuide(areaKawaguchiMotogo)} />
      ...
      <AreaDetailCta area={areaKawaguchiMotogo} />
    </>
  );
}
