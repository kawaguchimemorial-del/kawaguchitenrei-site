...
import { areaAngyo } from "@/lib/areas";
import { AreaLocalGuide } from "@/components/area/AreaLocalGuide";
import { buildLocalGuide } from "@/lib/area-local-guide";

const SITE_URL = "https://kawaguchitenrei.com";
const pageUrl = `${SITE_URL}/area/${areaAngyo.slug}/`;
const imageUrl = `${SITE_URL}/images/home/hall/hall-exterior.jpg`;

export const metadata: Metadata = {
  title: areaAngyo.metaTitle,
  description: areaAngyo.metaDescription,
  alternates: { canonical: `/area/${areaAngyo.slug}/` },
  openGraph: {
    title: areaAngyo.metaTitle,
    description: areaAngyo.metaDescription,
    url: `/area/${areaAngyo.slug}/`,
    type: "article",
    images: [
      {
...
const faqJsonLd = {
  "@context": "https://schema.org",
  "@type": "FAQPage",
  mainEntity: areaAngyo.faqs.map((f) => ({
    "@type": "Question",
    name: f.q,
    acceptedAnswer: {
      "@type": "Answer",
...
const funeralHomeJsonLd = {
  ...
  description:
    "植木の里として知られる川口市安行地区周辺で葬儀・家族葬・直葬・一日葬を承る葬儀社。自社式場「川口メモリアルホール」(川口市西新井宿)を中心に、ご搬送・ご安置・葬儀式・川口市めぐりの森での火葬まで一貫してサポートします。",
};
...
const relatedLinks = [
  ...
  {
    label: "戸塚安行の葬儀・家族葬",
    description: "戸塚安行駅周辺の葬儀・家族葬のご案内",
...
export default function AngyoAreaPage() {
  return (
    <>
      <script
      ...
      <AreaDetailIntro area={areaAngyo} />
      <AreaPrimaryHall
        area={areaAngyo}
        description="安行地区の住宅地から、お車でご来館いただける川口典礼の自社ホール。1日1組貸切の落ち着いた式場で、家族葬・一日葬に適しています。"
      />
      <AreaFeatures area={areaAngyo} />
      <AreaPlans area={areaAngyo} />
      <AreaSaijo area={areaAngyo} />
      <AreaReasons area={areaAngyo} />
      <AreaFlow area={areaAngyo} />
      <AreaFaq area={areaAngyo} />

      <AreaLocalGuide {...buildLocalGuide(areaAngyo)} />
