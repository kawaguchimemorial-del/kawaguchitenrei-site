...
import { areaTozukaAngyo } from "@/lib/areas";
import { AreaLocalGuide } from "@/components/area/AreaLocalGuide";
import { buildLocalGuide } from "@/lib/area-local-guide";

const SITE_URL = "https://kawaguchitenrei.com";
const pageUrl = `${SITE_URL}/area/${areaTozukaAngyo.slug}/`;
const imageUrl = `${SITE_URL}/images/home/hall/hall-exterior.jpg`;

export const metadata: Metadata = {
  title: areaTozukaAngyo.metaTitle,
  description: areaTozukaAngyo.metaDescription,
  alternates: { canonical: `/area/${areaTozukaAngyo.slug}/` },
  openGraph: {
    title: areaTozukaAngyo.metaTitle,
    description: areaTozukaAngyo.metaDescription,
    url: `/area/${areaTozukaAngyo.slug}/`,
    type: "article",
    images: [
      {
...
const faqJsonLd = {
  "@context": "https://schema.org",
  "@type": "FAQPage",
  mainEntity: areaTozukaAngyo.faqs.map((f) => ({
    "@type": "Question",
    name: f.q,
    acceptedAnswer: {
      "@type": "Answer",
...
const funeralHomeJsonLd = {
  ...
  description:
    "埼玉高速鉄道「戸塚安行駅」周辺で葬儀・家族葬・直葬・一日葬を承る葬儀社。自社式場「川口メモリアルホール」(川口市西新井宿)を中心に、ご搬送・ご安置・葬儀式・川口市めぐりの森での火葬まで一貫してサポートします。",
};
...
const relatedLinks = [
  ...
  },
  {
    ...
    description: "安行・戸塚安行エリアで利用できる寺院会館・民営式場の候補もあわせてご相談いただけます。",
...
export default function TozukaAngyoAreaPage() {
  return (
    <>
      <script
      ...
      <AreaDetailIntro area={areaTozukaAngyo} />
      <AreaPrimaryHall
        area={areaTozukaAngyo}
        description="戸塚安行駅周辺の住宅地から、お車でご来館いただける川口典礼の自社ホール。1日1組貸切の落ち着いた式場で、家族葬・一日葬に適しています。"
      />
      <AreaFeatures area={areaTozukaAngyo} />
      <AreaPlans area={areaTozukaAngyo} />
      <AreaSaijo area={areaTozukaAngyo} />
      <AreaReasons area={areaTozukaAngyo} />
      <AreaFlow area={areaTozukaAngyo} />
      <AreaFaq area={areaTozukaAngyo} />

      <AreaLocalGuide {...buildLocalGuide(areaTozukaAngyo)} />
      ...
      <AreaDetailCta area={areaTozukaAngyo} />
    </>
  );
}
